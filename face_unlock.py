...
while True:	
	...
	# Convert the face locations to the format (top, right, bottom, left)
	#converted_face_locations = [(location[0], location[1], location[2], location[3]) for location in X_face_locations]

	# If no faces are
	# in the image, return an empty result.


	if len(X_face_locations) != 0:
		
		# Find encodings for faces in the test iamge
		faces_encodings = face_recognition.face_encodings(image, known_face_locations=X_face_locations, num_jitters=1)
